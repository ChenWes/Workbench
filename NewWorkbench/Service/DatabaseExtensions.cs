...
namespace NewWorkbench.Service
{
    ...
    public static class DatabaseExtensions
    {
        ...
        private static IDbConnection DefaultConnection
        {
            get
            {
                return domainConfig.DefaultConnection;
            }
        }
        ...
        private static string DefaultConnectionString
        {
            get
            {
                return domainConfig.DefaultConnectionString;
            }
        }
                ...
                params object[] parameters)
        {
            ...
            using (IDataReader dataReader = cmd.ExecuteReader())
            {
                ...
                //2、DataReader转换Json
                string jsonstr = CommonLibrary.JsonConverter.ToJson(dataReader);
                dataReader.Close();
                dataReader.Dispose();
                cmd.Dispose();
                conn.Close();
                conn.Dispose();
                //3、Json转换动态类
                dynamic dyna = CommonLibrary.JsonConverter.ConvertJson(jsonstr);
                return dyna;
            }
        }
