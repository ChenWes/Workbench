...
namespace NewWorkbench.Service
{
    /// <summary>
    /// 数据操作基本实现类，公用实现方法
    ...
    public abstract class RepositoryBase<T> : IRepository<T> where T : class
    {
        ...
        private DbContext context = new domainConfig().db;
            ...
            where TEntity : class
        {
            ...
            return CommonLibrary.JsonConverter.JsonClass(list);
        }
        ...
        /// </summary>
        public virtual IList<T1> PageByListSql<T1>(string sql, IList<DbParameter> parameters, CommonLibrary.PageCollection page)
        {
            return null;
        }
        ...
        /// 待自定义分页函数，使用必须重写，
        /// </summary>
        public virtual IList<T> PageByListSql(string sql, IList<DbParameter> parameters, CommonLibrary.PageCollection page)
        {
            return null;
        }
        ...
        public virtual CommonLibrary.PageInfo<T> Query(IQueryable<T> query, int index, int PageSize)
        {
            if (index < 1)
            {
                index = 1;
            ...
            return new CommonLibrary.PageInfo<T>(index, PageSize, count, query.ToList());
        }
        ...
        /// <returns>自定义实体集合</returns>
        public virtual CommonLibrary.PageInfo<object> Query<TEntity, TOrderBy>
            ...
            where TEntity : class
        {
            ...
            //返回结果为null，返回所有字段
            if (selector == null)
                return new CommonLibrary.PageInfo<object>(index, pageSize, count, query.ToList<object>());
            return new CommonLibrary.PageInfo<object>(index, pageSize, count, selector(query).ToList());
        }
        ...
        /// <returns>结果集</returns>
        public virtual CommonLibrary.PageInfo Query(int index, int pageSize, string tableName, string field, string filter, string orderby, string group, params DbParameter[] para)
        {
            //执行分页算法
            if (index <= 0)
                index = 1;
            ...
            if (list != null)
                return new CommonLibrary.PageInfo(index, pageSize, count, list.Cast<object>().ToList());
            return new CommonLibrary.PageInfo(index, pageSize, count, new { });
        }
        ...
        /// <returns></returns>
        public virtual CommonLibrary.PageInfo Query(int index, int pageSize, string sql, string orderby, params DbParameter[] para)
        {
        ...
        /// 多表联合分页算法
        /// </summary>
        public virtual CommonLibrary.PageInfo Query(IQueryable query, int index, int PageSize)
        {
            ...
            return new CommonLibrary.PageInfo(index, PageSize, count, enumerable.ToList());
        }
