...
namespace NewWorkbench.Service.ServiceImp
{
    public class UserManage : RepositoryBase<Domain.SYS_USER>, IService.IUserManage
    {
        ...
        public Domain.SYS_USER UserLogin(string useraccount, string password)
        {
            ...
            //而不能通过再次加密输入的密码来匹配
            if (entity != null && new CommonLibrary.AESCrypt().Decrypt(entity.PASSWORD) == password)
            {
                return entity;
            }
        ...
        public bool IsAdmin(int userId)
        {
            ...
            return roles.ToList().Any(item => item.ID == CommonLibrary.Enums.ClsDic.DicRole["超级管理员"]);
        }
        ...
        {
            var cookie =CommonLibrary.CookieHelper.GetCookie("cookie_rememberme");
            if (cookie != null)
            {
                //验证json的有效性
                ...
                {
                    //解密
                    var cookievalue = new CommonLibrary.AESCrypt().Decrypt(cookie.Value);
                    //是否为json
                    if (!CommonLibrary.JsonSplit.IsJson(cookievalue)) return null;
                    try
                    {
                        var jsonFormat = CommonLibrary.JsonConverter.ConvertJson(cookievalue);
                        if (jsonFormat != null)
                        {
                            var users = UserLogin(jsonFormat.username, new CommonLibrary.AESCrypt().Decrypt(jsonFormat.password));
                            if (users != null)
